...
vi.mock('workflow-actions/catalog/resolve-inputs', () => ({
  resolveInputs: vi.fn(async (_ns: string, _def: unknown, stateId: string, database?: string) => {
...
describe('useWorkflowRun', () => {
  it('returns visible inputs excluding hidden', async () => {
    ...
    expect(lastDatabase).toBe('main')
  })

  it('passes custom database to resolveInputs', async () => {
    await useWorkflowRun('platform', { id: 'w', initial: 'form', states: {} }, 'form', 'admin')
    expect(lastDatabase).toBe('admin')
  })
})

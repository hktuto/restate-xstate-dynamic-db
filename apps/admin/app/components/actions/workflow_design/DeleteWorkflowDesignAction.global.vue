...
async function open() {
  ...
  const confirmed = confirm('Are you sure you want to delete this workflow design?')
  if (!confirmed) return

  loading.value = true
  try {
    const api = useApi()
    await api.fetch(`/api/admin/workflow-designs/${props.context.record.id}`, { method: 'DELETE' })
    props.context.refresh?.()
  } catch (err: any) {
    alert(err?.message ?? 'Failed to delete workflow design')
  } finally {
    loading.value = false
  }
}

...
onMounted(async () => {
  allowed.value = await can(props.context.resourceType as any, 'edit', props.context.record?.id as string | undefined)
})

function open(ctx?: ActionContext) {
  const context = ctx && 'record' in ctx ? ctx : props.context
  if (!context.record?.id) return
  navigateTo(`/workflow-designs/${context.record.id}`)
}

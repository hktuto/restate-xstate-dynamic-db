...
async function loadRunInputs() {
  ...
  if (!namespace) {
    runError.value = 'No company selected'
    return
  }

  try {
    const { visibleInputs } = await useWorkflowRun(namespace, config.value, rule.startState)
...
async function run() {
  ...
  try {
    const result = await api.fetch<{ id: string }>('/api/workflow-instances', {
      method: 'POST',
